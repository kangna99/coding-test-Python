...
배열 B에 있는 원소 하나를 골라서 두 원소를 서로 바꾸는 것을 말한다. 동빈이의 최종 목표는 배열 A의 모든 원소의 합이
최대가 되도록 하는 것이며, 여러분은 동빈이를 도와야 한다.

N, K, 그리고 배열 A와 B의 정보가 주어졌을 때, 최대 K번의 바꿔치기 연산을 수행하여 만들 수 있는 배열 A의
모든 원소의 합의 최댓갓을 출력하는 프로그램을 작성하시오.
...
"""
아이디어)
배열 A는 오름차순 정렬, 배열 B는 내림차순 정렬 후 
서로의 원소를 인덱스 순서대로 K번 바꾼 후 합을 구한다.
"""
